...
layout (location = 0) out vec4 ShadowResult;
...
void main() {
    ...
    for(int i = 0; i < numLights; ++i) {
        ...
        if(computePointLightShadow(FragPos,n,lightPos[i],shadowCubeMaps[i])==0.0) {
            diffuse = lightIntensity[i]/rr*max(0.f,dot(n,l));
            //Specular Caculation
            vec3 viewDir = normalize(eyePos - FragPos);
            vec3 reflectDir = reflect(-l, n);  
            spec = pow(max(dot(viewDir, reflectDir), 0.0), 64);
            specular = specularStrength * spec * lightIntensity[i];
        }
    ...
    LightResult += vec4(texture(ssaoTex,TexCoord).rgb*ambientLight*texture(gAlbedoSpec,TexCoord).rgb,1.f);  
